import numpy as np
import matplotlib
matplotlib.use('Agg')  # Required to save plots
import matplotlib.pyplot as plt
from sklearn.linear_model import LogisticRegression
from scipy.spatial.distance import cdist
import os

# plt.switch_backend('agg')  # Switch to non-interactive mode for plotting

result_dir = "results"
os.makedirs(result_dir, exist_ok=True)

def generate_ellipsoid_clusters(distance, n_samples=100, cluster_std=0.5):
    '''generates two clusters of data points in the shape of ellipsoids, then shifts them over
    on x and y axis by some distance'''
    np.random.seed(0)
...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        plt.plot(xx, slope * xx + intercept, 'k--', linewidth=2)
